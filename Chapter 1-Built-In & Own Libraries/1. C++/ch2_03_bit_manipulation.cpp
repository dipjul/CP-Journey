...
#define lowBit(S) (S & (-S)) // lowest position of 1 remains intact rest becomes 0
#define setAll(S, n) (S = (1 << n) - 1)


#define modulo(S, N) ((S) & (N - 1))   // returns S % N, where N is a power of 2
#define isPowerOfTwo(S) (!(S & (S - 1)))
#define nearestPowerOfTwo(S) ((int)pow(2.0, (int)((log((double)S) / log(2.0)) + 0.5))) // TODO
#define turnOffLastBit(S) ((S) & (S - 1))
#define turnOnLastZero(S) ((S) | (S + 1))
// 1010101011 -- elements previous to the last zero would remain same
// only from that index the bits would be flipped (for the one greater than the give number
// 1010101100
